...
> {
  ...
  public render(): JSX.Element {
    ...
    const points: IVerticalBarChartDataPoint[] = jsonData.data.seriesLabels.map(
      (label, index) => ({
        x: label[1], // Use country name as x
        y: jsonData.data.seriesCollapsed[index][0].value, // Use corresponding value as y
        ...(isChecked && {
          lineData: {
            y: jsonData.data.seriesCollapsed[index][0].value,
            yAxisCalloutData: 'Data',
          },
        }),
        callOutAccessibilityData: {
          ariaLabel: `Series ${index + 1} of ${
            jsonData.data.seriesLabels.length
          } ${label[1]} ${jsonData.data.seriesCollapsed[index][0].value}`,
        },
      })
    );
